...
export default function AdminDeviceRequests(): React.JSX.Element {
  ...
  const totalPages = show === -1 ? 1 : Math.ceil(filteredData.length / show);
    ...
    return (
      <div className="rounded-2xl p-4 mx-auto max-w-full mr-8"
        style={{
          background:
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[310px] rounded-lg shadow-lg custom-scroll">
        <table className="min-w-full text-white text-xs">
          <thead className="bg-[#0C1F3C] border-b border-gray-700 sticky top-0 z-10">
            <tr>
              {["ID", "Alamat", "Segmen", "Detail", "Koordinat", "Status", "Aksi"].map((header) => (
                <th key={header} className="px-4 py-3 text-left font-semibold uppercase tracking-wider">{header}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((r, index) => (
                ...
                >
                  ...
                  <td className="px-2 py-1">{r.lat}, {r.lng}</td>
      ...
      {show !== -1 && (
        <div className="flex justify-center mt-4 gap-1 flex-wrap text-xs">
          <button
            ...
            className={`px-3 py-1.5 rounded-full ${currentPage === 1 ? "bg-gray-400" : "bg-gray-600 hover:bg-gray-700 text-white"}`}
          >
            Previous
          </button>
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full ${currentPage === i + 1 ? "bg-blue-600 text-white" : "bg-gray-600 text-white hover:bg-gray-700"}`}
            >
              {i + 1}
            </button>
          ))}
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full ${currentPage === totalPages ? "bg-gray-400" : "bg-gray-600 hover:bg-gray-700 text-white"}`}
          >
            Next
          </button>
        </div>
