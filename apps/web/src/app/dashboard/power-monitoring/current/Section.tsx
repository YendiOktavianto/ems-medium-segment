// app/(dashboard)/dashboard/power-monitoring/current/Section.tsx
"use client";
...
export default function CurrentSection({ device }: { device?: Location }) {
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex flex-col items-center w-full md:w-1/2 bg-[#032d7a] rounded-2xl p-6">
          <svg viewBox="0 0 200 200" className="w-56 h-56">
            <path d={describeArc(100, 100, 80, -120, endAngle)} stroke="red" strokeWidth="7" fill="none" />
            <path d={describeArc(100, 100, 80, -160, -120)} stroke="yellow" strokeWidth="7" fill="none" />
            <path d={describeArc(100, 100, 80, startAngle, -160)} stroke="lime" strokeWidth="7" fill="none" />
          ...
          <div className="text-white px-3 py-1 rounded mt-[-81px] text-sm font-light">Current</div>
          <div className="bg-gray-200 text-black px-3 py-1 rounded mt-2 font-bold">{current} A</div>
        </div>

        {/* LINE CHART */}
        <div className="w-full bg-[#032d7a] rounded-xl p-4">
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={data} margin={{ top: 20, right: 20, bottom: 20, left: 30 }}>
              ...
              <YAxis
                ...
                label={{ value: "Current", angle: -90, position: "insideLeft", fill: "#fff", dy: 30 }}
              />
              <Tooltip
                contentStyle={{
