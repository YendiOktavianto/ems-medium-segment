// app/(dashboard)/dashboard/power-monitoring/frequency/Section.tsx
"use client";
...
export default function FrequencySection({ device }: { device?: Location }) {
  const [frequency, setFrequency] = useState(49);
  const [data, setData] = useState<{ time: string; frequency: number }[]>([]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const possibleFreq = [49, 50, 51];
      const newFrequency =
        possibleFreq[Math.floor(Math.random() * possibleFreq.length)];
      setFrequency(newFrequency);
      setData((prev) => [...prev, { time, frequency: newFrequency }].slice(-15));
    }, 2000);
  ...
  const percentage = (frequency - minVal) / (maxVal - minVal);
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex flex-col items-center w-full md:w-1/2 bg-[#032d7a] rounded-2xl p-6">
          <svg viewBox="0 0 200 200" className="w-56 h-56">
            <path d={describeArc(100, 100, 80, startAngle, endAngle)} stroke="lime" strokeWidth="7" fill="none" />
            {[49, 50, 51].map((val, i) => {
              const angle = startAngle + ((val - minVal) / (maxVal - minVal)) * angleRange;
          ...
          <div className="text-white px-3 py-1 rounded mt-[-81px] text-sm font-light">Frequency</div>
          <div className="bg-gray-200 text-black px-3 py-1 rounded mt-2 font-bold">{frequency} Hz</div>
        </div>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={data} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <YAxis
                ...
                label={{ value: "Frequency", angle: -90, position: "insideLeft", fill: "#fff", dy: 40 }}
              />
              <Tooltip
                contentStyle={{
