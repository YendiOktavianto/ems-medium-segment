// app/(dashboard)/dashboard/power-monitoring/power/Section.tsx
"use client";
...
export default function PowerSection({ device }: { device?: Location }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setPower(newPower);
      setData((prev) => [...prev, { time, power: newPower }].slice(-15));
    }, 2000);
  ...
  const percentage = (power - minVal) / (maxVal - minVal);
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex flex-col items-center w-full md:w-1/2 bg-[#032d7a] rounded-2xl p-6">
          <svg viewBox="0 0 200 200" className="w-56 h-56">
            ...
            <path d={describeArc(100, 100, 80, -60, 10)} stroke="yellow" strokeWidth="7" fill="none" />
            <path d={describeArc(100, 100, 80, startAngle, -60)} stroke="lime" strokeWidth="7" fill="none" />

            {Array.from({ length: 11 }).map((_, i) => {
              const val = i * (maxVal / 10);
              const angle = startAngle + (i / 10) * angleRange;
              const pt = polarToCartesian(100, 100, 65, angle);
              return <text key={i} x={pt.x} y={pt.y} textAnchor="middle" alignmentBaseline="middle" fontSize="8" fill="white">{val}</text>;
            })}

            {[0, 400, 800, 1200, 1600, 2000, 2400, 2800, 3200, 3600, 4000].map((val, i) => {
        ...
        <div className="w-full bg-[#032d7a] rounded-xl p-4">
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={data} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <YAxis domain={[0, 4000]} ticks={[0,400,800,1200,1600,2000,2400,2800,3200,3600,4000]} tick={{ fill: "#fff", fontSize: 9}} label={{ value: "Power", angle: -90, position: "insideLeft", fill: "#fff", dy:20 }} />
              <Tooltip
                contentStyle={{
